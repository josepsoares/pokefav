import {
  personalityFavorites,
  personalityTeams
} from '../assets/content/personalityContent';
...
const getStatsValues = (firstArray, secondArray, stats) => {
  for (let x = 0; x < firstArray.length; x++) {
    for (const [index, secondItem] of secondArray.entries()) {
      stats[index].value.push(secondItem.base_stat);
      const sum = stats[index].value.reduce((a, b) => a + b, 0);
...
const compareStats = (array, personality) => {
  ...
  if (personality === 'favorites') {
    for (const resultStats of personalityFavorites) {
      returnResults.push(
        new verdictFromStats(resultStats[0], resultStats[1], resultStats[2])
      );
    }
  } else {
    for (const resultStats of personalityTeams) {
      returnResults.push(
        new verdictFromStats(resultStats[0], resultStats[1], resultStats[2])
      );
    }
...
const getStatsMessages = (array, type) => {
  if (array?.length === 0) {
    ...
    const arrayStats = getStats(array[0].stats);
