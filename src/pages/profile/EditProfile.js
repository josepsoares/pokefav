...
class EditProfile extends Component {
  ...
  handleSelectChange = (value, action) => {
    if (value.value === 'Male') {
      this.setState({
        [action.name]: { label: value.value, value: value.value },
  ...
  printListText = text => {
    return (
      <ListItem style={{ fontSize: '13px' }} className="text-left py-2">
        {text}
      </ListItem>
    )
  }
            ...
            {selectGender.value === 'Female'
              ? femaleAvatars.map((item, key) => (
                  <Col
                    ...
                    key={key}
                    onClick={() => {
                      this.handleAvatarChange(item)
                    }}
                  >
                    <img
                      alt={item}
                      className={`avatar pb-2 ${
              ...
              <Paper>
                <List style={listStyle}>
                  {favoritePokemons.length !== 0
                    ...
                    : this.printListText(
                        "You still haven't added any Favorite Pokémons to your list yet! Search for your favorites in the PokéList!"
                      )}
                </List>
              </Paper>
            </Col>
                  ...
                  {favoriteTeam.length !== 0
                    ? favoriteTeam.map((item, key) => (
                        <ListItem
                          ...
                          key={key}
                          className="justify-content-center align-items-center w-75 mx-auto mb-2"
                        >
                          <img
                            style={{ minHeight: '40px' }}
                            alt={item.name}
                    ...
                    : this.printListText(
                        "You still haven't added any Pokémon to your Favorite Team! Search for your team members in the PokéList!"
                      )}
                </List>
              </Paper>
            </Col>
                    ...
                    {friends.length !== 0
                      ? friends.map((item, key) => (
                          <ListItem
                            ...
                            key={key}
                            className="col-10 mx-auto justify-content-center align-items-center px-2"
                          >
                      ...
                      : this.printListText(
                          "You still haven't added any Pokémon Trainers to your follow list! Check cool trainers on the PokéTrainers tab!"
                        )}
                  </Row>
                </List>
              </Paper>
        ...
        >
          <ModalBody className="pt-4 px-4 pb-0">
            <p className="text-center m-0">
              Are you sure you want to remove <b>{modalItemName}</b> from your{' '}
              {modalAction}?
            </p>
          </ModalBody>
          <ModalFooter className="py-4 justify-content-center text-center">
            <Button
              className="btn-warning w-25 mx-3"
              onClick={() => {
                this.toggleModal()
            ...
            <Button
              className="btn-danger w-25 mx-3"
