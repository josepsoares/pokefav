  ...
  return (dispatch) => {
    ...
    Promise.all(urls.map(url =>
      ...
      .then((data) => {
        ...
        var arrayAlternateForms = [];
        for (let item of data[1].varieties) {
          if (!item.is_default) {
            arrayAlternateForms.push(item.pokemon.url)
          }
        }

        Promise.all(arrayAlternateForms.map(url => fetch(url)
          .then((response) => response.json().then((json) => response.ok ? json : Promise.reject(json)))))
          .then((data) => dispatch({ type: 'POKEMONINFO_ALTERNATEFORM_DATA_SUCCESS', payload: data }))
          .catch((error) => dispatch({ type: 'POKEMONINFO_ALTERNATEFORM_DATA_ERROR', error: error }))
        ...
        fetch(data[1].evolution_chain.url)
          .then((response) => {
            return response.json().then(function (json) {
              ...
              console.log(data)
              dispatch({ type: 'POKEMONINFO_EVOLUTION_DATA_SUCCESS', payload: data })
            })
...
export const getYoutubeVideo = (pokemon) => {
  return (dispatch) => {
    ...
    fetch(url)
      ...
      .catch((error) => dispatch({ type: 'YOUTEBE_DATA_ERROR', payload: undefined, errorYtData: error }))
  }
}
